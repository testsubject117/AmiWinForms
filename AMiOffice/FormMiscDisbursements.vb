...
' Matt Sprague Request #2 — Disbursement Listing
' View modes: Detail | By Payee | By Year
' NOTE: Legacy CHECK.INV stores payee as a customer code (e.g. "NATAIRCR"), not a full name.
Public Class FormMiscDisbursements
    Inherits Form
    ...
    Private WithEvents rbPayee      As New RadioButton()
    Private WithEvents rbPayeeDetail As New RadioButton()
    Private WithEvents rbYear       As New RadioButton()
    Private lblStatus            As New Label()

    Private Class CheckBlockEx
        Public Property Block As CheckInvBlock
        Public Property ParsedDate As DateTime
    End Class

    Private _filtered As List(Of CheckBlockEx)

    Public Sub New()
        Me.Text = "Disbursement Listing"
    ...
    Private Sub BuildUI()
        ...
        StyleRadio(rbPayee, "By Payee", New Point(140, 78), False)
        StyleRadio(rbPayeeDetail, "Payee Detail", New Point(240, 78), False)
        StyleRadio(rbYear, "By Year", New Point(370, 78), False)

        lblStatus.Text = "Select date range and click Load."
        lblStatus.ForeColor = Color.Cyan : lblStatus.BackColor = Color.Black
        lblStatus.AutoSize = True : lblStatus.Location = New Point(460, 84)
        ...
        Me.Controls.AddRange({lblTitle, lblFrom, dtpFrom, lblTo, dtpTo,
                               ...
                               lblView, rbDetail, rbPayee, rbPayeeDetail, rbYear,
                               lblStatus, grid})
    End Sub
    ...
    Private Sub SetDetailColumns()
        grid.Columns.Clear()
        grid.Columns.Add(New DataGridViewTextBoxColumn() With {.Name = "colCheck",    .HeaderText = "Check #",      .Width = 90})
        grid.Columns.Add(New DataGridViewTextBoxColumn() With {.Name = "colDate",     .HeaderText = "Date",         .Width = 100})
        grid.Columns.Add(New DataGridViewTextBoxColumn() With {.Name = "colPayee",    .HeaderText = "Payee Code",   .Width = 150})
        grid.Columns.Add(New DataGridViewTextBoxColumn() With {.Name = "colSalesman", .HeaderText = "Salesman",     .Width = 90})
        grid.Columns.Add(New DataGridViewTextBoxColumn() With {.Name = "colInvoices", .HeaderText = "Invoices Paid",.Width = 300})
    ...
    Private Sub SetGroupColumns(groupHeader As String)
        ...
        grid.Columns.Add(New DataGridViewTextBoxColumn() With {.Name = "colCount", .HeaderText = "# Checks",    .Width = 100,
            .DefaultCellStyle = New DataGridViewCellStyle() With {.Alignment = DataGridViewContentAlignment.MiddleRight}})
    ...
    Private Sub btnLoad_Click(sender As Object, e As EventArgs) Handles btnLoad.Click
        ...
        Try
            lblStatus.Text = "Loading CHECK.INV ..." : Application.DoEvents()
            Dim allBlocks = CheckInvReader.ReadAll(LegacyDataPaths.CheckInv)
            Dim fromDate = dtpFrom.Value.Date : Dim toDate = dtpTo.Value.Date

            _filtered = allBlocks.Select(Function(b)
                                             Dim d As DateTime = Date.MinValue
                                             DateTime.TryParseExact(b.DateText, "MM-dd-yyyy",
                                                 System.Globalization.CultureInfo.InvariantCulture,
                                                 System.Globalization.DateTimeStyles.None, d)
                                             Return New CheckBlockEx With {.Block = b, .ParsedDate = d}
                                         End Function).
                Where(Function(x) x.ParsedDate >= fromDate AndAlso x.ParsedDate <= toDate).
                OrderBy(Function(x) x.ParsedDate).ThenBy(Function(x) x.Block.CheckNumber).ToList()
    ...
    Private Sub RefreshView()
        ...
        If rbPayee.Checked Then
            ShowByPayee()
        ElseIf rbPayeeDetail.Checked Then
            ShowPayeeDetail()
    ...
    Private Sub ShowDetail()
        ...
        For Each x In _filtered
            Dim b = x.Block
            grid.Rows.Add(b.CheckNumber, b.DateText, b.CustomerCode, b.SalesmanCode, String.Join(", ", b.Invoices), b.Amount.ToString("N2"))
            total += b.Amount
        Next
        AddTotalRow(True, "GRAND TOTAL", _filtered.Count & " checks", total)
        lblStatus.Text = $"{_filtered.Count} checks  |  Grand Total: {total:C}  |  {dtpFrom.Value:MM/dd/yyyy} – {dtpTo.Value:MM/dd/yyyy}"
    End Sub

    Private Sub ShowByPayee()
        SetGroupColumns("Payee Code")
        grid.Rows.Clear()
        Dim groups = _filtered.GroupBy(Function(x) x.Block.CustomerCode.Trim()).OrderBy(Function(g) g.Key).ToList()
        Dim grandTotal As Decimal = 0D
        Dim grandCount As Integer = 0
        For Each g In groups
            Dim amt = g.Sum(Function(x) x.Block.Amount)
            Dim cnt = g.Count()
            grid.Rows.Add(g.Key, cnt.ToString("N0"), amt.ToString("N2"))
            grandTotal += amt
            grandCount += cnt
        Next
        AddTotalRow(False, "GRAND TOTAL", grandCount.ToString("N0"), grandTotal)
        lblStatus.Text = $"{groups.Count} payees  |  {grandCount} checks  |  Grand Total: {grandTotal:C}"
    End Sub

    Private Sub ShowPayeeDetail()
        SetDetailColumns()
        grid.Rows.Clear()
        Dim payeeGroups = _filtered.GroupBy(Function(x) x.Block.CustomerCode.Trim()).OrderBy(Function(g) g.Key).ToList()
        Dim grandTotal As Decimal = 0D
        Dim grandCount As Integer = 0

        For Each pg In payeeGroups
            ' ── Payee header ─────────────────────────────────────────
            AddBandRow(pg.Key, 2, Color.FromArgb(20, 60, 20), Color.LightGreen)

            Dim payeeTotal As Decimal = 0D
            Dim payeeCount As Integer = 0

            ' ── Year groups within this payee ─────────────────────────
            Dim yearGroups = pg.GroupBy(Function(x) x.ParsedDate.Year).OrderBy(Function(g) g.Key).ToList()
            For Each yg In yearGroups
                ...
                For Each x In yg.OrderBy(Function(e) e.ParsedDate)
                    ...
                    grandCount += 1
                Next
                AddSubtotalRow("    " & yg.Key.ToString() & " Subtotal", yg.Count() & " checks", yearTotal, Color.FromArgb(15, 45, 45), Color.Cyan)
            Next

            ' ── Payee total ───────────────────────────────────────────
            AddSubtotalRow(pg.Key & "  —  Total", payeeCount & " checks", payeeTotal, Color.FromArgb(40, 40, 40), Color.Yellow)
        Next

        AddTotalRow(True, "GRAND TOTAL", grandCount & " checks", grandTotal)
        lblStatus.Text = $"{payeeGroups.Count} payees  |  {grandCount} checks  |  Grand Total: {grandTotal:C}"
    End Sub
    ...
    Private Sub ShowByYear()
        ...
        Next
        AddTotalRow(False, "GRAND TOTAL", grandCount.ToString("N0"), grandTotal)
        lblStatus.Text = $"{groups.Count} years  |  {grandCount} checks  |  Grand Total: {grandTotal:C}"
    End Sub

    Private Sub AddTotalRow(isDetail As Boolean, label As String, countOrCol1 As String, total As Decimal,
                            Optional col4 As String = "", Optional col5 As String = "")
        Dim idx = grid.Rows.Add()
        Dim row = grid.Rows(idx)
        If isDetail Then
            row.Cells(0).Value = label
            row.Cells(4).Value = countOrCol1
            row.Cells(5).Value = total.ToString("N2")
            row.Cells(5).Style.Alignment = DataGridViewContentAlignment.MiddleRight
        Else
            row.Cells(0).Value = label
            row.Cells(1).Value = countOrCol1
    ...
    End Sub
    Private Sub rbPayee_CheckedChanged(s As Object, e As EventArgs) Handles rbPayee.CheckedChanged
        If rbPayee.Checked Then RefreshView()
    End Sub
    Private Sub rbPayeeDetail_CheckedChanged(s As Object, e As EventArgs) Handles rbPayeeDetail.CheckedChanged
        If rbPayeeDetail.Checked Then RefreshView()
    End Sub
    ...
    Private Sub btnExport_Click(sender As Object, e As EventArgs) Handles btnExport.Click
        Dim viewName As String = If(rbPayee.Checked, "ByPayee", If(rbPayeeDetail.Checked, "PayeeDetail", If(rbYear.Checked, "ByYear", "Detail")))
        Using sfd As New SaveFileDialog()
            sfd.Title = "Export Disbursement Listing"
            sfd.Filter = "CSV Files (*.csv)|*.csv"
            sfd.FileName = $"Disbursements_{viewName}_{dtpFrom.Value:yyyyMMdd}_{dtpTo.Value:yyyyMMdd}.csv"
    ...
    Private Sub btnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click
        ...
        doc.DocumentName = "Disbursement Listing"
        _printRowIndex = 0
    ...
    Private Sub PrintDoc_PrintPage(sender As Object, e As PrintPageEventArgs)
        Dim g = e.Graphics
        Dim colWidths() As Integer = {70, 80, 150, 80, 200, 80}
        ...
        If _printRowIndex = 0 Then
            g.DrawString("DISBURSEMENT LISTING", titleFont, Brushes.Black, x0, y)
            y += 22
            Dim x = x0
